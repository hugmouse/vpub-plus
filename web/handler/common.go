...
<body>
    <header>
        <span><a href="{{ root }}/">home</a> <a href="{{ root }}/posts">posts</a> <a href="{{ root }}/feed.atom">atom</a></span>
        <nav>
            {{ if logged }}
            {{ if .hasNotifications }}<a href="{{ root }}/notifications" class="notifications">New replies</a> {{ end }} <a href="{{ root }}/account">{{ .logged.Name }}</a> (<a href="{{ root }}/logout">logout</a>)
            {{ else }}
